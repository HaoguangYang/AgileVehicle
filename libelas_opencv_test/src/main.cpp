...
void vediosetup(int *camid)
{
	extern VideoCapture cap[sizeof(camid)];
	for (int i = 1; i <= sizeof(camid); i++)
	{
...
void process(int * camid, string camcalibfile) {
...
#pragma omp parallel for
  ...
  if (I[1]->width()<=0 || I[1]->height() <=0 || I[2]->width()<=0 || I[2]->height() <=0 ||
      I[1]->width()!=I[2]->width() || I[1]->height()!=I[2]->height()) {
    cout << "ERROR: Images must be of same size, but" << endl;
    cout << "       I1: " << I[1]->width() <<  " x " << I[1]->height() << 
                 ", I2: " << I[2]->width() <<  " x " << I[2]->height() << endl;
    delete I;
    //delete I2;
    return;    
  }
  ...
  int32_t width  = I[1]->width();
  int32_t height = I[1]->height();
  ...
  Elas elas(param);
  elas.process(I[1]->data,I[2]->data,D1_data,D2_data,dims);
